...
formRegister?.addEventListener('submit', (e) => {
    ...
    let lista = getUsers(); if (lista.some(u => u.documento === documento)) return alert("Este documento ya está registrado.");
    lista.push({ documento, contrasena, nombre, casa, parqueadero: "Sin parqueadero asignado", saldo: "$0 (Al día)" });
    localStorage.setItem('usuariosPropietarios', JSON.stringify(lista)); alert("¡Registro Exitoso!"); formRegister.reset(); tabLogin.click();
});
...
function renderParq() {
    ...
    getUsers().filter(u => u.documento !== "admin").forEach(u => {
        ...
        const tr = document.createElement('tr');
        tr.innerHTML = `<td><strong>${pts[0] || u.parqueadero}</strong><br><span class="status-badge ${tiene ? 'status-green' : 'status-orange'}">${tiene ? 'Asignado' : 'Disponible'}</span></td><td><strong>${u.casa}</strong><br><small>${u.nombre}</small><br><small style="color:#c53030;font-weight:bold;">${pts[1] || 'Sin vehículo registrado'}</small></td><td><button class="btn-manage" style="background:#2e7d32;margin-bottom:4px;width:100%;" onclick="goParq('${u.documento}')">Plaza</button><br><button class="btn-manage" style="background:#0288d1;width:100%;" onclick="goCarros('${u.documento}')">Vehículo</button></td>`;
...
window.goParq = (doc) => {
    ...
    const n = prompt(`Asignación para la ${list[i].casa}\nAsignación actual: ${list[i].parqueadero}\n\nEscriba el parqueadero disponible o 'Sin parqueadero asignado':`);
